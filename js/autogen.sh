...
# bootstrap autotools
#
# $Id: autogen.sh,v 1.1.1.1 2006/10/27 10:36:49 smoln Exp $

ag_unused_vars='DEFS CPPFLAGS LDFLAGS LIBS'
ag_unused_dirs='sbindir libexecdir sysconfdir sharedstatedir 
...
ag_cmd_autoheader()
{
  ...
  local ag_src_jsconfig_h_in='src/jsconfig.h.in'
  ...
  # Print everything which goes to src/jsconfig.h.in from config.h.in
  ag_run_shell $ag_sed -n \
    '1 i\#ifndef jsconfig_h___\
...
\
     ...
     }' $ag_config_h_in >$ag_src_jsconfig_h_in

  # Remove everything which goes to src/jsconfig.h.in from config.h.in
  ag_run_shell $ag_sed -i \
    '1 a\
\
#ifndef config_h___\
