...
#ifdef DEBUGGER
#include "jsdebug.h"
#ifdef DEBUGGER_JAVA_UI
#include "jsdjava.h"
#endif /* DEBUGGER_JAVA_UI */
#ifdef DEBUGGER_C_UI
#include "jsdb.h"
#endif /* DEBUGGER_C_UI */
#endif /* DEBUGGER */
...
ValueToScript(JSContext *cx, jsval v)
{
    ...
    if (!JSVAL_IS_PRIMITIVE(v) &&
        JS_GET_CLASS(cx, JSVAL_TO_OBJECT(v)) == JS_GetScriptClass()) {
            ...
            int32 *ip)
{
    ...
    *ip = 0;
    if (argc != 0) {
        ...
        intarg = 0;
        if (!JSVAL_IS_PRIMITIVE(v) &&
            (JS_GET_CLASS(cx, JSVAL_TO_OBJECT(v)) == JS_GetFunctionClass() ||
             JS_GET_CLASS(cx, JSVAL_TO_OBJECT(v)) == JS_GetScriptClass())) {
...
#if defined(SHELL_HACK) && defined(JS_DEBUG) && defined(XP_UNIX)
static JSBool
