...
js_FileRead(JSContext *cx, JSFile *file, jschar *buf, int32 len, int32 mode)
{
    ...
    switch (mode) {
      case ASCII:
        ...
        count = js_BufferedRead(file, (char *)aux, len);
        if (count == -1) {
            JS_free(cx, aux);
            return 0;
        }
      ...
      case UTF8:
        ...
        for (count = 0;count<len;count++) {
            i = js_BufferedRead(file, (char *)utfbuf+remainder, 3-remainder);
            if (i<=0) {
                return count;
            }
...
file_toURL(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    urlChars = js_InflateString(cx, (char *)url, &urlLength);
    if (urlChars == NULL) return JS_FALSE;
    *rval = STRING_TO_JSVAL(js_NewString(cx, urlChars, urlLength, 0));
