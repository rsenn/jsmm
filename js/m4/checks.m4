...
# AC_CHECK_LIB(LIBRARY, FUNCTION, [PROLOGUE], [MESSAGE],
#              [ACTION-IF-FOUND], [ACTION-IF-NOT-FOUND],
#              [LIBRARY-PATHS])
AC_DEFUN([AC_CHECK_CXXLIBRARY],
[AC_LANG_PUSH([C++])
m4_ifval([$5], , [AH_CHECK_LIB([$1])])dnl
AS_LITERAL_IF([$1],
        [AS_VAR_PUSHDEF([ac_Lib], [ac_cv_lib_$1_$2])],
        [AS_VAR_PUSHDEF([ac_Lib], [ac_cv_lib_$1''_$2])])dnl
AC_CACHE_CHECK(m4_ifval([$4], [$4], [for $2 in -l$1]), ac_Lib,
[ac_check_lib_save_LIBS=$LIBS
LIBS="$7 -l$1 $LIBS"
...
LIBS=$ac_check_lib_save_LIBS])
AS_IF([test AS_VAR_GET(ac_Lib) = yes],
      [m4_default([$5], [AC_DEFINE_UNQUOTED(AS_TR_CPP(HAVE_LIB$1))
  LIBS="-l$1 $LIBS"
])],
      [$6])dnl
AS_VAR_POPDEF([ac_Lib])dnl
AC_LANG_POP([C++])])# AC_CHECK_LIB
