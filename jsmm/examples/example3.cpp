...
/*          output << rect->getX() << ", ";
  ...
  Bool ConstructRectangle
    (ContextRef cx, ObjectPtr obj, Uint argc, Value argv[], Value &rval)
  {
    ObjectAPI robj = obj;
    Example::Rectangle *rect;
    
    std::cout << "Constructor: obj.getClass().getName(): " << ClassAPI(robj.getClass(cx)).getName() << std::endl;
    ...
    std::cerr << "ctor private: " << ObjectAPI(obj).getPrivate(cx) << std::endl;
...
int main()
{
  try
  {
    ...
    ConstructorObject ctor(cx, global, 0, RectangleClass, 
                           ConstructRectangle, 4, proto);
